...
def inject_globals():
    def img_url(path_or_key):
        """Convert image path/key to a full URL usable in templates."""
        if not path_or_key:
            return url_for("static", filename="product_images/placeholder.png")
        if path_or_key.startswith("http"):
            return path_or_key          # already a full URL
        if path_or_key.startswith("img/"):
            parts = path_or_key.split("/")
            if len(parts) == 3:
                return url_for("serve_product_image",
                               product_id=int(parts[1]), slot=int(parts[2]))
        return url_for("static", filename=path_or_key)   # local static file
...
# ═══════════════════════════════════════════════════════════
def doc_to_dict(doc):
    """Convert a MongoDB document to a plain dict with id as int (seq_id)."""
    if doc is None:
        return None
    d = dict(doc)
    d["id"] = doc.get("seq_id", str(doc.get("_id","")))
    d["_id"] = str(d.get("_id",""))
    return d
...
def init_db():
    ...
    # 1. Fix the Email Index conflict safely
    # This checks if the index exists and has the wrong settings before dropping it
    try:
        if "email_1" in db.users.index_information():
                db.users.drop_index("email_1")
        app.logger.info("🗑️ Dropped conflicting email index")
    except Exception as e:
        app.logger.warning(f"Index drop issue: {e}")

    # 2. Create the indexes
    # We use 'background=True' so it doesn't lock your database while working
    db.users.create_index("username", unique=True)
    db.users.create_index("email")
    db.users.create_index("phone")
    
    # 3. Handle the Products index
    # If this fails, it means you have products with duplicate 'seq_id's.
    try:
        db.products.create_index("seq_id", unique=True)
    except Exception as e:
        app.logger.warning(f"⚠️ Could not create unique index on seq_id: {e}")
        app.logger.warning("This usually means you have duplicate seq_ids in your data.")

    db.products.create_index("category")
    db.products.create_index([("name","text"),("category","text")])
    db.cart.create_index([("user_id",1),("product_id",1)])
    db.wishlist.create_index([("user_id",1),("product_id",1)], unique=True)
    db.orders.create_index("order_ref", unique=True)
    db.orders.create_index("user_id")
    db.recently_viewed.create_index([("user_id",1),("product_id",1)], unique=True)
...
def get_user_id():
    ...
    # Use the username from session to find the user
    user = col("users").find_one({"username": session.get("user")})
    if user:
        # Check if seq_id exists, otherwise fallback to the MongoDB _id
        uid = user.get("seq_id") or str(user["_id"])
        session["user_id"] = uid
        return uid
    return None
...
def insert_sample_products():
    if col("products").count_documents({}) > 0:
        return
    # Initialise counter
...
def home():
    ...
    trending = list(col("products").find({"trending":1}).sort([("rating",-1),("reviews",-1)]).limit(12))
    trending = [doc_to_dict(p) for p in trending]
    season_picks = []
    if sc:
        season_picks = [doc_to_dict(p) for p in col("products").find({"category":{"$in":sc}}).sort("rating",-1).limit(8)]
    deals_pool = [doc_to_dict(p) for p in col("products").find({"badge":{"$ne":None}}).sort("reviews",-1).limit(20)]
...
def share_product(pid):
    ...
    product_images=p.get("images") or get_product_images(pid)
    img_url=(url_for("static",filename=product_images[0],_external=True)
             if product_images and not product_images[0].startswith("http")
...
# ═══════════════════════════════════════════════════════════
@app.route('/img/<int:pid>/<int:slot>')
def img(pid, slot):
    path = f"static/product_{pid}_slot_{slot}.png"
    
    if os.path.exists(path):
        return send_file(path)
    else:
        return send_file("static/default.png")
# ═══════════════════════════════════════════════════════════
# ADMIN
# ═══════════════════════════════════════════════════════════
...
def admin_edit_product(pid):
    ...
    # your existing form updates here...

        for slot in range(1, 7):
            file = request.files.get(f'image_{slot}')
        
            if file and file.filename != '':
                filepath = f"static/product_{pid}_slot_{slot}.png"
                file.save(filepath)
